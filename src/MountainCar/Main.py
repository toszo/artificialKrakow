...
class MainClass:
    ...
    def chooseActionBasedOnIndex(self, qValues, state):
        ...
        if (valuesCount != 0 and str(state) in self.episodeIndex.keys() and len(self.episodeIndex[str(state)]) > sum(
                indexValues) / valuesCount):
    ...
    def runEpisode(self, env, observation, q, discreter):
        ...
        while not done:
            ...
            self.saveEpisode(
                {'observation': observation, 'action': action, 'newObservation': newObservation, 'reward': reward,
                 'done': done}, discreter)
    ...
    def learnFromPreviousExperience(self, q, discreter):
        for _ in range(len(self.episodeIndex.keys()) * 10):
            ...
            q.learn(discreter.getState(episode['observation']), episode['action'],
                    discreter.getState(episode['newObservation']), episode['reward'], episode['done'])
